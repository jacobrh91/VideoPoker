...
public class HandAnalyzer {
	...
	Pattern myPattern = Pattern.compile("[^CDHS]+");
	Matcher myMatcher = myPattern.matcher(item);
	myMatcher.find();
	String cardVal = myMatcher.group();
	int cardFinal;
	switch (cardVal) {
	case "2":
	    cardFinal = 2;
	...
	if (straight && flush) {
	    ...
	    //              14 + 13 + 12 + 11 + 10 = 60
	    if (sum == 60) hand_outcome = "Royal Flush";
	    else hand_outcome = "Straight Flush";
	}
	...
	if (modeValue[1] == 4) hand_outcome = "Four-of-a-Kind";
	//If three, check for full house
	    ...
	    if (others[0] == others[1]) hand_outcome = "Full House";
	    else hand_outcome = "Three-of-a-Kind";
	}
	//If two, check for two pair
		...
		else pair_value = cValues;
	    }
	    if (o[0] == o[1] || o[0] == o[2] || o[1] == o[2]) {
		hand_outcome = "Two Pairs";
	    }
	    else {
		if (pair_value >= 11) hand_outcome = "One Pair: Royal";
		else hand_outcome = "One Pair: Low";
	    }
	}
